...
export class Game {
  ...
  startRun(who, opts = {}) {
    ...
    // Passable so a fixture can pin a layout exactly; rolled otherwise and
  ...
  onUltimate(player) {
    ...
    // A column of light where the player is standing.
    for (let i = 0; i < 26; i++) {
   ...
   *
   * The live array rather than a copy, and that matters: it is read by every
   * skill, every projectile step and the aim assist, sixty times a second,
   * with thirty enemies in it.
   *
   * Kept as a method taking a `who` it does not currently read, because every
   * targeting helper in the game already goes through here — which is what
   * made it a single edit to teach a fireball, a chain lightning and a
   * whirlwind about a new kind of opponent, and will be again.
   */
  hostiles() {
    return this.mobs;
  }
  ...
  enemiesInRadius(x, y, z, radius, who = this.player) {
    ...
    return this.hostiles().filter((m) => !m.dead && dist2(x, y, z, m.x, m.centerY, m.z) <= r2);
  }
  ...
  dealDamage(source, target, amount, opts = {}) {
    ...
    // character; everything the *screen* does belongs to the one whose eyes
    // the camera is behind. They are the same entity now that there is only
    // ever one player, and the split is kept because collapsing it is how the
    // bug it was introduced to fix got written in the first place.
